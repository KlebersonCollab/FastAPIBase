...
async def create_refresh_token(user: User):
    ...
    refresh_token_obj = await RefreshToken.create(
...
async def verify_refresh_token(token: str):
    refresh_token_obj = await RefreshToken.get_or_none(token=token).prefetch_related(
        "user"
    )
    if not refresh_token_obj or refresh_token_obj.expires_at < datetime.now(
        timezone.utc
    ):
        raise HTTPException(
