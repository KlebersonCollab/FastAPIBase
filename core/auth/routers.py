...
async def logout(body: RefreshTokenIn):
    ...
    if refresh_token_obj:
        ...
        logger.info(
            "Refresh token revogado e adicionado à blacklist",
            token=body.refresh_token,
            user_id=refresh_token_obj.user_id,
        )
    else:
        logger.warning(
            "Tentativa de revogar refresh token inexistente", token=body.refresh_token
        )
...
@router.post(
    ...
    dependencies=[Depends(check_permissions([Permissions.MANAGE_ROLES]))] + rate_limit,
)
...
@router.get(
    ...
    dependencies=[Depends(check_permissions([Permissions.MANAGE_ROLES]))],
)
